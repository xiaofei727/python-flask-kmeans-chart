...
function makeGraphs(error, projectsJson, statesJson, skillsJson) {
	...
	function fnRedrawRadar(d){
		...
		jobs = new Array();
		var y_ind = 0;
		var kSum_count = ndx.groupAll().reduceSum(function(d) {
			for(var i=0;i<skills.length;i++){
				var key = skills[i];
				if(d[key] != null && d[key] > 0){
					data_x.push(i);
					var p = jobs.indexOf(d['Job']);
					if(p == -1){
						data_y.push(y_ind);
						y_ind ++;
						jobs.push(d['Job']);
					...
					// console.log("exist skill " + y_ind + ":" + i);
				}
			}
			return 1;
		}).value();
		console.log("job Count: ", y_ind);
		job_count = y_ind;
		data = [data_x, data_y];
		refresh(data);
